...
class HelixPlanner(KernelBase):
    ...
    def break_down_goal(self, goal, tool_catalog):
        ...
        result = query_llm(sys_prompt, user_prompt, self.llm, prefix="Overall")
    ...
    def revise_execution_path(self, reason, active_task, tools_meta):
        ...
        feedback = query_llm(sys_prompt, user_prompt, self.llm)
    ...
    def _build_graph(self, structure):
        ...
        for label, props in structure.items():
            self.task_total += 1
            self.node_map[label] = WorkflowUnit(label, props['description'], props['type'])
    ...
    def _insert_task_node(self, patch_data, parent_task):
        ...
        for label, content in patch_data.items():
            self.task_total += 1
            self.node_map[label] = WorkflowUnit(label, content['description'], content['type'])
