import os
import logging
from strata.prompts.strata_pt import templates
import json
#from strata.utils.utils import self_learning_print_logging, get_project_root_path, read_json, save_json
from strata.utils.utils import mostly_printable, get_repo_root, export_to_json, import_from_json
class SelfLearning:
    """
    ...
    def __init__(self, agent, learner_cls, tool_manager, config, text_extractor_cls=None):
        ...
        self.learner = learner_cls(templates['self_learning_prompt'], tool_manager)
        self.course = {}
    ...
    def _initialize_learning(self, software_name, package_name, demo_file_path):
        ...
        mostly_printable(self.config)

        # Ensure the courses directory exists
        courses_dir = os.path.join(get_repo_root(), 'courses')
        ...
        if os.path.exists(prior_course_path):
            self.course = import_from_json(prior_course_path)
        else:
            export_to_json(prior_course_path, {})
            self.course = {}
        ...
        if demo_file_path:
            if not os.path.isabs(demo_file_path):
                demo_file_path = os.path.join(get_repo_root(), demo_file_path)
    ...
    def self_learning(self, software_name, package_name, demo_file_path):
        ...
        export_to_json(prior_path, new_course)
